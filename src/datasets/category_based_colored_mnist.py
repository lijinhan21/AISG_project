...
class CategoryBasedColoredMNIST(BaseMNISTDataset):
    """
    Category-based ColoredMNIST dataset that creates environments from VAE-based categorization.
    ...
    def __init__(self, root='./data', env='train', transform=None, target_transform=None):
        ...
        self.prepare_category_based_colored_mnist()
        
        if env in ['train', 'val', 'test']:
            self.data_label_env_tuples = self._load_prepared_data('category_based_colored_mnist', env)
        elif env == 'all_train':
            self.data_label_env_tuples = self._load_prepared_data('category_based_colored_mnist', 'all_train')
        else:
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, target, env) where target is the binary label and env is the category-based environment
        """
        ...
        # Reshape image from flattened to 3x28x28
        if len(img.shape) == 1:
    ...
    def prepare_category_based_colored_mnist(self):
        """Prepare the category-based ColoredMNIST dataset from categorized data."""
        if self._check_prepared_data('category_based_colored_mnist', ['train.pt', 'val.pt', 'test.pt']):
            print('Category-based ColoredMNIST dataset already exists')
            return

        print('Preparing Category-based ColoredMNIST dataset from categorized data')
        
        # Load categorized training data
        categorized_data_path = os.path.join('./categorized_data/ColoredMNIST/train_categorized_data.npz')
        ...
        category_dir = self._create_data_dir('category_based_colored_mnist')
        ...
        print(f"Category-based ColoredMNIST dataset created with {len(torch.unique(env_labels))} environments")
